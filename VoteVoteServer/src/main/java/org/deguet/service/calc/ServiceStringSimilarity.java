...
public class ServiceStringSimilarity {
	...
	private final int deleteCost, insertCost, replaceCost, swapCost;
	   ...
	   */
	  public ServiceStringSimilarity(int deleteCost, int insertCost,
	  ...
	  public ServiceStringSimilarity(){
		  this(1,1,1,1);
	  }
	...
	public String best(String source, List<String> candidates) {
		String result = candidates.get(0);
		int score = this.distance(source, result);
		for (String candidate : candidates){
			if (distance(source, candidate) < score){
				score = distance(source, candidate);
				result = candidate;
			}
		}
		return result;
